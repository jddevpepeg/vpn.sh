...
# Fill up YOUR_USERNAME, YOUR_PASSWORD, YOUR_IPSEC_PSK if you need a definite value for each variable, else the system will generate

echo -n "Enter the VPN username:  "; read YOUR_USERNAME
YOUR_PASSWORD=''
YOUR_IPSEC_PSK=''

if [[ $YOUR_USERNAME == '' ]]; then YOUR_USERNAME=`echo $RANDOM | md5sum | head -c 8`; fi
if [[ $YOUR_IPSEC_PSK == '' ]]; then YOUR_IPSEC_PSK=`echo $RANDOM | md5sum | head -c 8`; fi
if [[ $YOUR_PASSWORD == '' ]]; then YOUR_PASSWORD=`echo $RANDOM | md5sum | head -c 8`; fi
